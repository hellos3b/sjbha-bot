...
/** @typedef {import('./bastion').Bastion} Bastion */
...
class Route {
    ...
    ignore(channels, bastion) {
        ...
        console.log("Ignoring channels", channels)
    ...
    restrict(channels, bastion) {
        ...
        console.log("Restricting channels to ", channels)
    ...
    addAction(action, route) {
        if (!route.options && this.__options) {
    ...
    async exec(context) {
        ...
        if (this.channels && this.channels.size && process.env.NODE_ENV === "production") {
            if (!this.channels.has(context.channelID)) {
                console.log(`   `, chalk.magenta(`[${(this.command || this.action)}]`), chalk.gray("Used outside of restricted channels"), { restrict: this.channels, context: context.channelID })
        ...
        if (this.channelsIgnore && this.channelsIgnore.size) {
        ...
        try {
            ...
            if (e.statusCode === 400) {
                ...
                if (content.includes("Must be 2000")) {
                 this.send(context.channelID, "<:ohno:469564483329720340> Unable to send, message is more than 2000 characters")
                } 
            } else {
                this.send(context.channelID, `<:ohno:469564483329720340> Oops something random broke... "\`${e.statusMessage}\`"`)
            }
        }
    }
