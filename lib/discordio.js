...
DCP.deleteMessage = function(input, callback) {
	deleteMessage(this, input.channelID, input.messageID, callback);
};
 ...
 * @arg {Snowflake} input.serverID
 * @arg {String} input.name
 * @arg {String} [input.type] - 'text', 'voice' or 'category': defaults to 'text'.
...
DCP.editChannelPermissions = function(input, callback) { //Will shrink this up later
	...
	if (type(input.allow) === 'array') {
		input.allow.forEach(function(perm) {
			if (allowed_values.indexOf(perm) < 0 && !input.ignoreAllowedValues) return;
	...
	} else if (typeof(input.allow) === 'number') {
...
function deleteMessage(client, channel, messageID, callback) {
	resolveID(client, channel, function(channelID) {
		client._req('delete', Endpoints.MESSAGES(channelID, messageID), function(err, res) {
...
function Member(client, server, data) {
	...
	['username', 'discriminator', 'bot', 'avatar', 'game'].forEach((k) => {
		...
		Object.defineProperty(Member.prototype, k, {
			...
			set: function(v) { 
				if (!this.id) return;
				if (!client.users[this.id]) return;

				client.users[this.id][k] = v; 
			},
			enumerable: true,
		});
	});
