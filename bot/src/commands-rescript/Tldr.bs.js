...
function listRecentTldrs(interaction) {
  ...
  return StdLib.P.map(tldrs, (function (tldrs) {
                ...
                var embed = new DiscordJs.MessageEmbed().setTitle("💬 TLDR").setColor(11393254);
                Curry._2(StdLib.A.forEach, tldrs._0, (function (tldr) {
                        var value = "*" + StdLib.$$Date.fromNow(tldr.timestamp) + " • " + tldr.from + " • <#" + tldr.channelID + ">*";
...
function saveNewTldr(interaction) {
  ...
  return StdLib.P.map(savedTldr, (function (tldr) {
                if (tldr.TAG === /* Ok */0) {
                  var embed = new DiscordJs.MessageEmbed().setColor(11393254).setDescription("📖 " + tldr._0.message);
                  return {
                          TAG: /* Embed */1,
                          _0: embed,
                          _1: /* Public */0
